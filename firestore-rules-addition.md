...
**⚠️ 2026-08-04 更正這個問題的真正原因：** 原本以為只是「對方部署時忘記保留我們這段」，一直重貼了事。後來查了 Firebase Rules API 的部署歷史才發現：**衛生營出勤系統那邊自己也已經實作了「新增勤務」頁挑選今日前支任務的功能**（跟這個 session 更早討論、你核准的整合方案是同一個功能——見 `index.html` 裡對方寫的 `__openForwardSupportPicker`／`window.__createTask`），並且在他們自己維護的規則裡也寫了一個 `match /forward_support_requests/{id}` 區塊，用的是完全不同的欄位層級權限模型（不是我們這邊 `isFsrDispatcher()`/`isFsrApplicant()` 這套角色判斷）。

兩邊各自部署時，Firestore 規則整份覆蓋，只會有一份存在——不是單純「對方漏掉我們的」，是**兩邊各自維護了互相衝突的版本，誰後部署誰就贏**，導致兩邊系統輪流出現權限錯誤（我們這邊看到「查不到」「permission-denied」，對方那邊部署我們的版本時大概也會反過來出問題）。

**已經處理：** 把對方需要的權限（`isActive()` 讀取全部案件、把已指派案件改成 `filled` 並寫入車長/駕駛/車號、`claimedTaskId` 防止同單位多人重複佔用同一筆）直接併進下面第 5 版的 `forward_support_requests` 區塊裡（語意照抄對方 2026-08-04 部署的版本），現在只要是我們這份規則生效，兩邊系統都能正常運作，不用再互相覆蓋。**但如果對方那邊之後又改了 `forward_support_requests` 的權限需求（例如新增欄位），這份文件不會自動知道，需要重新比對兩邊。**
